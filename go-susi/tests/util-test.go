...
func Util_test() {
  ...
  ts, err := util.AddTimestamp("20170201120000", "-3_seconds")
  check(ts, "20170201115957")
  check(err,nil)
  ts, err = util.AddTimestamp("20170201120000", "60_seconds")
  check(ts, "20170201120100")
  check(err,nil)
  ts, err = util.AddTimestamp("20170201120000", "-61_minutes")
  check(ts, "20170201105900")
  check(err,nil)
  ts, err = util.AddTimestamp("20170201120000", "6_minutes")
  check(ts, "20170201120600")
  check(err,nil)
  ts, err = util.AddTimestamp("20170201120000", "-1_hours")
  check(ts, "20170201110000")
  check(err,nil)
  ts, err = util.AddTimestamp("20170201120000", "2_hours")
  check(ts, "20170201140000")
  check(err,nil)
  ts, err = util.AddTimestamp("20170201120000", "-1_days")
  check(ts, "20170131120000")
  check(err,nil)
  ts, err = util.AddTimestamp("20170201120000", "28_days")
  check(ts, "20170301120000")
  check(err,nil)
  ts, err = util.AddTimestamp("20170201120000", "-2_weeks")
  check(ts, "20170118120000")
  check(err,nil)
  ts, err = util.AddTimestamp("20170201120000", "4_weeks")
  check(ts, "20170301120000")
  check(err,nil)
  ts, err = util.AddTimestamp("20170201120000", "-2_months")
  check(ts, "20161201120000")
  check(err,nil)
  ts, err = util.AddTimestamp("20170201120000", "12_months")
  check(ts, "20180201120000")
  check(err,nil)
  ts, err = util.AddTimestamp("20170201120000", "-2_years")
  check(ts, "20150201120000")
  check(err,nil)
  ts, err = util.AddTimestamp("20170201120000", "3_years")
  check(ts, "20200201120000")
  check(err,nil)
  
  ts, err = util.AddTimestamp("20170201120000", "61 minutes")
  check(ts, "20170201120000")
  check(err,"Invalid timestamp offset: \"61 minutes\"")
  ts, err = util.AddTimestamp("20170201120000", "2_knackwürschtl")
  check(ts, "20170201120000")
  check(err,"Unknown timestamp offset unit: \"knackwürschtl\"")
  ts, err = util.AddTimestamp("20170201120000", "apoa_minutes")
  check(ts, "20170201120000")
  check(err,"Invalid timestamp offset: \"apoa_minutes\": strconv.ParseInt: parsing \"apoa\": invalid syntax")
