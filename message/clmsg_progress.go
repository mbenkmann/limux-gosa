...
func clmsg_progress(xmlmsg *xml.Hash) {
  ...
  util.Log(1, "INFO! Progress info from client %v/%v: %v",macaddress,xmlmsg.Text("source"), progress)
  ...
  if progress == "100" {
    util.Log(1, "INFO! Progress 100% => Setting status \"done\" for client v/%v",macaddress,xmlmsg.Text("source"))
